...
export RAY_TMPDIR="${RAY_TMPDIR:-/tmp/ray_mlopsfull_ci}"
...
python -c "from madewithml.config import logger; logger.info('ci logging smoke test'); print('logging ok')"

python -m madewithml.train \
  --experiment-name ci-smoke-test \
  ...
  --num-epochs 1 \
  --batch-size 8 \
  --results-fp results/ci_train_results.json
